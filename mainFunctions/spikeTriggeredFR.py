...
def spikeTriggeredFR(spikeTimesNeuron1,spikeTimesNeuron2,sessionStart,sessionEnd,\
                            preWindowLen = 100,postWindowLen = 100, figureShow =0, yLim=0):
    ...
    if figureShow:
        ...
        ax.plot(crossCorrTimeVector,np.ones(len(smoothedCrossCorr))/lenWindowCrossCorr,'w--')

        ax.set_xlabel('delay (ms)')
        if yLim:
            ax.set_ylim(top=yLim/lenWindowCrossCorr)
        ax.text(.75*postWindowLen,\
        ax.get_ylim()[1]*0.9,'p=%(number)0.1e'%{'number':ks_Pval},ha='center')
        yAxisLable = [0]
        for tickTemp in range(1,max((ax.get_yticks()*lenWindowCrossCorr).astype('int'))+1):
            yAxisLable.append(str(tickTemp)+'C')
        ax.set_yticks(np.arange(len(yAxisLable))/lenWindowCrossCorr)
        ax.set_yticklabels(yAxisLable)

        # ax.set_ylabel('h (Chance Level)')
